################################################################################
# Name:    Bruce Adams
# Class:   CS2318-251 (Assembly Language, Spring 2017)
...
####################(3)####################
					addi $a0, $sp, 28	# insert vals to do string address to $a0
					jal GetOneIntByVal	# call function
					sw $v0, 105($sp)		# store return value in $v0 into 105($sp)

#      ValidateInt(&valsToDo, 1, 7, adjMsg);

####################(4)####################
					addi $a0, $sp, 105	# load integer value stored in 105($sp) into $a0
					li $a1, 1		# put 1 into $a1 (min int bounds)
					li $a2, 7		# put 7 into $a2 (max int bounds)
					addi $a3, $sp, 0	# put "is bad" msg address into $a3 
					jal ValidateInt		# call function
#      for (i = valsToDo; i > 0; --i)

####################(1)####################
					lw $t1, 105($sp)	# load value of integer into $t1 ($t1 is i)
					j FTestM1
begFBodyM1:					
#         if (i % 2) // i is odd
					andi $t0, $t1, 0x00000001
					beqz $t0, ElseI1	# if even goto ElseI1
					
#            intArr[valsToDo - i] = GetOneIntByVal(entIntPrompt);

####################(8)####################			# i is odd, so process
					addi $a0, $sp, 41 	# "Enter an integer" string address to $a0
					jal GetOneIntByVal
					lw $t3, 105($sp)	# $t3 = valsToDo
					sub $t4, $t3, $t1	# $t4 = valsToDo – i
					sll $t4, $t4, 2
					addi $t6, $sp, 77
					add $t4, $t4, $t6
					sw $v0, 0($t4)
...
####################(7)####################			# i is even
					lw $a0, 105($sp)		# $a0 = valsToDo
					sub $a0, $a0, $t1	# valsToDo - i
					sll $a0, $a0, 2		# multiply i by 4 to get address correct
					addi $a0, $a0, 77	# load address of array 	
					add $a0, $a0, $sp	# be sure to have $sp at correct address
					addi $a1, $sp, 41	# "Enter an integer" string address to $a0
					jal GetOneIntByAddr	# call function to get integer
...
####################(3)####################
					addi $a0, $sp, 77	# $a0 now has arr[0]
					lw $a1, 105($sp)	# load value of integer into $a1 ($a1 now has i)					
					addi $a2, $sp, 18	# $a2 has label address for initlab					
					jal ShowIntArray
					
#      for (i = 0, j = valsToDo - 1; i < j; ++i, --j)
####################(3)####################
					li $t1, 0		# make $t1 0
					lw $t2, 105($sp) 	# load valsToDo in $a0
					addi $t2, $t2, -1
					j FTestM2
begFBodyM2:
...
####################(5)####################
					sll $a1, $t1, 2		# multiply by 4 to get correct address
        				addi $a1, $a1, 77	# load address of array[]
        				add $a1, $a1, $sp	# correct address of array[]
        				sll $a2, $t2, 2		# multiply by 4 to get correct address
        				addi $a2, $a2, 77	# load address of array[]
        				add $a2, $a2, $sp	# correct address of array[]
 					jal SwapTwoInts		# Call function and  return
...
####################(3)####################
					addi $a0, $sp, 77	# $a0 now has arr[0]
					lw $a1, 105($sp)	# load value of integer into $a1 ($a1 now has i)					
					addi $a2, $sp, 66	# $a2 has label address for initlab
					jal ShowIntArray
					
#      GetOneCharByAddr(&reply, dmPrompt);

####################(2)####################
					addi $a0, $sp, 76	# value for reply
					addi $a1, $sp, 56	# load string address for "do more?"
...
####################(1)####################
					lb $v1, 76($sp)		# store address of 'reply' in $v1
					li $t0, 'n'
...
####################(4)####################
       					...
       					lw $t0, 0($a1) 		# intPtr1 ($a1) into $t0
        				lw $t3, 0($a2) 		# intPtr2 ($a2) into $t3
        				sw $t3, 0($a1) 		# intPtr2 ($a2) where intPtr1 ($a1) was
        				sw $t0, 0($a2) 		# intPtr1 ($a1) where intPtr2 ($a2 was
					jr $ra
