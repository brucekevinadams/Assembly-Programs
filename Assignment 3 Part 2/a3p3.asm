...
#                ProcArrays(&used3, a1, a2, a3, &used1, &used2);
					addi $a0, $sp, 172			# used 3
					addi $a1, $sp, 184			# a1[0]
					addi $a2, $sp, 232			# a2[0]
					addi $a3, $sp, 280			# a3[0]
					
					########## (4) ##########
					addi $t0, $sp, 180			# contents of used1 to $t0
					sw $t0, 16($sp)				# store at arg5
					addi $t0, $sp, 176			# address of used2
					sw $t0, 20($sp)				# store at arg6
...
#v1 will hold value at sIntArray
					sll $t9, $a3, 2 
					add $t9, $t9, $a2 	#$t9 = $v1 + $t9 ($a3 mult by 4)
...
#             anchor = (*begPtr);
					lw $s2, 0($a0)		# anchor must use $s2
#             remCount = 0;
					move $v0, $zero
#             //for (hopPtr22 = begPtr + 1; hopPtr22 < endPtr; ++hopPtr22)
#             hopPtr22 = begPtr + 1;
					addi $t2, $a0, 4	# hopPtr22 must use $t2
#             goto F1_PALATest;
					j F1_PALATest
begF1_PALA:#//{
#                //if (*hopPtr22 == anchor)
...
#             endPtr2 = intArr + (*usedPtr);
					lw $t0, 0($a1) 
					sll $t0, $t0, 2 	# $v1 = $v1 * 4
					add $s2, $t0, $a0 	# $s2 must be endPtr2 
					
#             //for (hopPtr2 = intArr; hopPtr2 < endPtr2; ++hopPtr2)
#             hopPtr2 = intArr;
					move $t2, $a0		# must use $t2 for hopPtr2
#             goto F_PALTest;
					j F_PALTest
begF_PAL:#//  {
...
#             hopPtr = begPtr; 
					move $t1, $a0				# must use $t1 for hopPtr
#             goto F_RAOTest;
					j F_RAOTest
begF_RAO:#//  {
#                //if (*hopPtr == target)
...
#             *used3Ptr = 0;
					sw $zero, 0($a3)
#             hopPtr1 = a1;
					move $t1, $a0		# must use $t1 for hopPtr1
#             endPtr1 = a1 + (*used1Ptr);
					lw $t0, 0($a1)
					sll $t0, $t0, 2
					add $t9, $t0, $a0	# must use $t9 for endPtr1
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#//void ProcArrays(int& used3, int a1[], int a2[], int a3[], int& used1, int& used2)
#void ProcArrays(int* used3Ptr, int a1[], int a2[], int a3[], int* used1Ptr, int* used2Ptr)
#{
###############################################################################
ProcArrays:
